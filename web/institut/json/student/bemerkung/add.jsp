...
<%@ page contentType="text/json" pageEncoding="UTF-8" import="de.shj.UP.data.StudentBemerkung,de.shj.UP.data.shjCore" session="true" isThreadSafe="false" errorPage="../../error.jsp"%><jsp:useBean id="user" scope="session" class="de.shj.UP.data.Dozent" /><jsp:useBean id="student" scope="session" class="de.shj.UP.logic.StudentData" /><jsp:useBean id="seminar" scope="session" class="de.shj.UP.logic.SeminarData" />
...
<%  StudentBemerkung d = new StudentBemerkung();
    ...
    d.setStudentBemerkungDatum(seminar.g_TODAY);
    d.setStudentBemerkungText(request.getParameter("bemerkung").trim());
    d.setStudentBemerkungTag("Config-New");
    ResultSet rTmp = d.sqlQuery("select max(\"lngStudentBemerkungID\")+1 from \"tblBdStudentBemerkung\";"); 
    if( !rTmp.next() ) {} // Wert bleibt "0"
    else d.setStudentBemerkungID(rTmp.getLong(1));
    rTmp.close();
